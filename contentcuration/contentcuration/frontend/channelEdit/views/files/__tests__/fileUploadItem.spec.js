...
describe('fileUploadItem', () => {
  describe('render', () => {
    it("'Unknown filename' should be displayed if original_filename is 'file'", () => {
      ...
      expect(wrapper.findComponent('[data-test="file-link"]').text()).toBe('Unknown filename');
    });
    ...
    it("original_filename should be displayed if its value is not 'file'", () => {
      ...
      const wrapper = makeWrapper({}, file);
      expect(wrapper.findComponent('[data-test="file-link"]').text()).toBe('SomeFileName');
    });
  ...
  describe('props', () => {
    it('should show the remove icon only if allowFileRemove', () => {
      ...
      expect(noRemoveWrapper.findComponent('[data-test="remove"]').exists()).toBe(false);

      const allowRemoveWrapper = makeWrapper({ allowFileRemove: true });
      expect(allowRemoveWrapper.findComponent('[data-test="remove"]').exists()).toBe(true);
    });
  });
  ...
  describe('methods', () => {
    ...
    it('clicking remove icon should emit a remove event', async () => {
      await wrapper.setProps({ allowFileRemove: true });
      wrapper.findComponent('[data-test="remove"]').vm.$emit('click');
